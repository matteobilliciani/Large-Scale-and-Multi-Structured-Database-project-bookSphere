"""
SCRIPT 1: MongoDB Data Generation - FINAL V5 (SUMMARY & NO REDUNDANT ID)
- Priority: BookCrossing Popularity > Amazon Popularity.
- Data Quality: Metadata from Amazon.
- Schema Changes:
    1. Removed root 'book_id' from reviews (it's inside book_snapshot).
    2. Added 'summary' to reviews (populated from Amazon 'review/summary').
    3. Strict ObjectIds everywhere.
    4. Smart Genres & Empty Bookshelf Filtering.
"""
...
# --- MAIN ETL ---
print("="*60 + "\nMONGO GENERATOR V5 (Summary & Clean Schema)\n" + "="*60)
...
if cp:
    ...
    books_data, books_map, title_to_book_id, isbn_to_book_id, authors_map, genres_set, book_id_to_genres, amz_id_to_book_id = \
        cp['books_data'], cp['books_map'], cp['title_to_book_id'], cp['isbn_to_book_id'], cp['authors_map'], cp['genres_set'], cp['book_id_to_genres'], cp['amz_id_to_book_id']
else:
    ...
    bc_books.rename(columns={'ISBN': 'isbn', 'Book-Title': 'title', 'Title': 'title'}, inplace=True)
    ...
    for _, row in top_books.iterrows():
        ...
        auth_name = parse_authors(row.get('authors'))[0]
        
        if auth_name not in authors_map:
            authors_map[auth_name] = {"id": generate_object_id(), "name": auth_name, "books": []}
        ...
        book = {
            ...
            "author": {"id": to_mongo_oid(authors_map[auth_name]["id"]), "name": auth_name},
            "genres": genres,
            "external_ids": {"isbns": []}, 
        ...
        authors_map[auth_name]["books"].append({"_id": to_mongo_oid(mid), "title": book["title"]})
    ...
    # --- PRE-CALCOLO PER WANT TO READ (Mappa Genere -> Lista ID Libri) ---
    genre_to_books_map = {}
    for b in books_data:
        bid_str = b['_id']['$oid'] # ID come stringa semplice
        for g in b['genres']:
...
if cp:
    ...
    for uid in uid_list:
        ...
        users_data[uid] = {
            ...
            "favorite_genres": [], "bookshelf": [], "reviews_year": []
        }
...
for _, row in bc_valid.iterrows():
    ...
    rev = {
        ...
        # RIMOSSO "book_id" alla radice
        "rating": rating, 
        "source": "bookcrossing", 
        "text": "",
        "summary": "", # Vuoto per BC
        "created_at": to_mongo_date(rdate), 
        "likes_count": 0, "is_banned": False,
        "book_snapshot": {"title": "Placeholder", "book_id": to_mongo_oid(mongo_bid)}
    }
...
for _, row in amz_valid.iterrows():
    ...
    rev = {
        ...
        "summary": summary, # Popolato per Amazon
        "created_at": to_mongo_date(rdate), 
...
# Fix Titles
bid_to_title = {b['_id']['$oid']: b['title'] for b in books_data}
for r in reviews_data:
    # Qui devo usare il path nidificato perché ho tolto book_id dalla radice
    # Ma nel dictionary python è accessibile direttamente dentro 'book_snapshot'
    bid = r['book_snapshot']['book_id']['$oid']
    if bid in bid_to_title: r['book_snapshot']['title'] = bid_to_title[bid]
...
# 1. USERS ENRICHMENT (SMART GENRES)
for uid, act in user_activity.items():
    ...
    if user_genres_counter:
        u["favorite_genres"] = [g for g, _ in user_genres_counter.most_common(3)]
    else:
        u["favorite_genres"] = random.sample(list(genres_set), random.randint(1, 3))

    # ... (codice esistente: popolamento favorite_genres) ...

    # 1. Popola i libri LETTI (Codice esistente, assicurati sia così)
    u["bookshelf"] = [
        {"book_id": to_mongo_oid(b), "status": "read", "added_at": to_mongo_date(random_date(2023, 2025))} 
        for b in list(act['read_books'])[:20]
    ]

    # --- NUOVO: AGGIUNGI 'WANT TO READ' (Max 2, Basato sui generi) ---
    # Trova candidati basati sui generi preferiti dell'utente
    candidates = set()
    for g in u["favorite_genres"]:
        if g in genre_to_books_map:
            candidates.update(genre_to_books_map[g])
    
    # Rimuovi i libri che l'utente ha già letto (act['read_books'] contiene ID stringa)
    valid_candidates = list(candidates - act['read_books'])
    
    # Pesca a caso max 2 libri
    if valid_candidates:
        ...
        for bid_str in picked_books:
            u["bookshelf"].append({
                ...
                "status": "want_to_read",
                # Data recente (fine 2025) per simulare interesse attuale
...
# STEP 5: SAVING (With Empty User Filter)
print("\n[5/6] Saving Files...")
...
# --- MODIFICA: CALCOLA STATISTICHE MA SALVA TUTTI ---
...
print(f"    User Stats: {active_count} active (with books) | {empty_count} empty (lurkers). Total: {len(all_users)}")

# SAVE ALL USERS (Include anche quelli con bookshelf vuota)
