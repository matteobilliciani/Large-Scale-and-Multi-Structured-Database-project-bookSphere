...
import it.unipi.bookSphere.repository.neo4j.projections.BookLikeProjection;
...
public interface BookNodeRepository extends Neo4jRepository<BookNode, String> {
    ...
    @Query("""
        ...
        """)
    List<BookLikeProjection> getLikedBooksByUser(@Param("userId") String userId);
    ...
    @Query("""
        ...
        """)
    List<BookLikeProjection> getLikedBooksByUser(
        @Param("userId") String userId, 
    ...
    // ========== ANALYTICS METHODS ==========
     ...
     /**
     * Calculate Internationality Index for a book
     * Returns the geographical distribution of likes and reviews for the book
     */
    @Query("""
        MATCH (b:Book {mongoId: $bookId})
